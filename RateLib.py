...
class reaction:
    ...
    def add_specie(self, filename, specie=None, marcus=False):
        ...
        if filename.endswith('.txt'):
            ...
            while i < len(lines):
                line = lines[i].strip()
                if line == 'SCF Done':
                    specie['SCF'] = float(lines[i+1])
                    i += 2
                elif line == 'Deg. of freedom':
                    specie['DoF'] = int(lines[i+1])
                    i += 2
                elif line == 'NAtoms':
                    specie['NAtoms'] = int(lines[i+1])
                    i += 2
                elif line == 'Pressure (in Pascal)':
                    specie['P'] = float(lines[i+1])
                    i += 2
                elif line == 'Molecular mass (in amu)':
                    specie['mass'] = float(lines[i+1])
                    specie['masskg'] = specie['mass'] * 1.660538921E-27
                    i += 2
                elif line == 'Symmetry number':
                    specie['NSym'] = float(lines[i+1])
                    i += 2
                elif line == 'Rotational temperature (Kelvin)':
                    specie['Trot'].extend(map(float, lines[i+1].split()))
                    i += 2
                elif line == 'Vibrational temperatures (Kelvin)':
                    i += 1
                    while lines[i].strip() != 'Electronic':
                        specie['Tvib'].extend(map(float, lines[i].split()))
                        i += 1
                elif line == 'Electronic':
                    specie['Qe'] = float(lines[i+1])
                    i += 2
                elif line == 'Zero-point correction':
                    specie['zpe'] = float(lines[i+1])
                    i += 2
                elif line == 'Sum of electronic and zero-point Energies':
                    specie['En'] = float(lines[i+1])
                    i += 2
                elif line == 'Sum of electronic and thermal Enthalpies':
                    specie['Ent'] = float(lines[i+1])
                    i += 2
                elif line == 'Sum of electronic and thermal Free Energies':
                    specie['EnG'] = float(lines[i+1])
                    i += 2
                elif line == 'Volume':
                    specie['rVol'] = float(lines[i+1])
                    i += 2
                elif specie is self.ts:
                    if line == 'Imaginary Frequency (in cm-1)':
                        specie['freqi'] = float(lines[i+1])
                    i += 2
                else:
                    i += 1  
        else:    
    ...
    def run_reaction(self, marcus=False):
        ...
        self.reaction['wigner'] = 1.0+(1.0/24.)*(self.h*self.reaction['freqi']*self.c/(self.kb*self.T))**2
        self.reaction['kwigner'] = self.reaction['wigner'] * self.reaction['ktst']

        self.reaction['Lnkwigner'] = np.log(self.reaction['kwigner'])
        self.reaction['Logkwigner'] = np.log10(self.reaction['kwigner'])
