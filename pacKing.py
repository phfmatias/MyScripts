...
class pacKing:
    def __init__(self):
        ...
        self.print_file()
        print('There are {} molecules in the box, {} charges were changed, and the system has a total net charge of {} e'.format(len(self.solves) + len(self.solutes), len(self.chargeToPrint), sum(float(i) for i in self.chargeToPrint)))
    ...
    def control(self):
        ...
        for i in range(len(arq)):
            if 'solvent' in arq[i]:
                self.solves.append(arq[i].split()[1])
                self.numbers.append(int(arq[i].split()[2]))
            if 'solute' in arq[i]:
                self.solutes.append(arq[i].split()[1])
                self.numbers.append(int(arq[i].split()[2]))
            if 'data' in arq[i]:
                self.data = arq[i].split()[1]
            if 'box_size' in arq[i]:
                self.box_size = arq[i].split()[1]
    ...
    def change_charges(self):
        ...
        for i in range(len(list_of_molecules)):            
            for j in range(start, start + self.numbers[i] * self.atomnumbers[i]):
                ...
                self.chargeToPrint.append(self.charges[i][c])
                count += 1
    ...
    def get_charges(self):
        ...
        if sum(float(i) for i in self.charges_dict.values()) != 0:
            qValue = sum(float(i) for i in self.charges_dict.values())
            # make the sum be equal to 0, adding or removing the difference to the last charge
            for i in self.charges_dict.keys():
                if i == list(self.charges_dict.keys())[-1]:
                    self.charges_dict[i] = str(float(self.charges_dict[i]) - qValue)
